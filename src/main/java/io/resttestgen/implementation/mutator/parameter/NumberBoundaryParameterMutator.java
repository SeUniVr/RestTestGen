...
public class NumberBoundaryParameterMutator extends ParameterMutator {

    private static final Logger logger = LogManager.getLogger(NumberBoundaryParameterMutator.class);
    ...
    public Parameter mutate(Parameter parameter) {
        ...
        if (isParameterMutable(numberParameter)) {
            if (Environment.getInstance().getRandom().nextBoolean()) {
                numberParameter.setValueManually(numberParameter.getMinimumRepresentableValue());
            } else {
                numberParameter.setValueManually(numberParameter.getMaximumRepresentableValue());
            }
        } else {
