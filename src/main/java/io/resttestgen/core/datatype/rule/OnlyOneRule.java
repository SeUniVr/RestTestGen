...
public class OnlyOneRule extends Rule {

    public OnlyOneRule(HashSet<ParameterName> parameterNames) {
        super(RuleType.ONLY_ONE, parameterNames);
    }
    ...
    /**
     * This rule is applicable only if one OnlyOne rule is present in the combination. Moreover, other IPDs that
    ...
    public boolean isApplicable(Operation operation, List<Rule> combination) {
        if (combination.stream().filter(r -> r.getRuleType() == RuleType.ONLY_ONE).count() > 1) {
            return false;
        }

        Set<ParameterName> otherIpdsParameterNames = new HashSet<>();
        combination.stream().filter(r -> r.getRuleType() == RuleType.ALL_OR_NONE || r.getRuleType() == RuleType.OR ||
    ...
    public void apply(Operation operation) {
        operation.getOnlyOne().add(parameterNames);
    }

    @Override
    public boolean isApplied(Operation operation) {
        return operation.getOnlyOne().contains(parameterNames);
    }
